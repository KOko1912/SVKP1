...
export default function Pagina() {
  ...
  // Activa fondo temático (igual que en Perfil/Configuración)
  useEffect(() => {
    document.body.classList.add("vendor-theme");
    return () => document.body.classList.remove("vendor-theme");
  }, []);

  // Propaga tema (gradiente) a variables CSS globales
  useEffect(() => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!rt.ok) throw new Error(dt?.message || "No se pudo cargar la tienda");
        setTienda(dt);
  ...
  return (
    <div className="vendedor-container">
      ...
      {/* Si el layout trae un bloque 'hero', lo renderizamos desde el layout.
          Si NO lo trae, dejamos un Hero por defecto para no perder la cabecera. */}
      {Array.isArray(blocks) && blocks.some(b => b.type === "hero")
        ? null
      ...
      <div className="poster-body">
        ...
        {conPrecio ? (
          <div className="poster-price">
            ...
            {p.precioOriginal && (
              <span className="original-price">${Number(p.precioOriginal).toFixed(2)}</span>
            )}
          </div>
        ) : (
          <div className="poster-price">
            <span className="badge">Ver variantes</span>
          </div>
        )}
      </div>
    </article>
